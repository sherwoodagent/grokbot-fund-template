...
For **Track B** (crypto asymmetric — owner opt-in only; see `docs/advanced-growth.md`), also run skill `crypto-asymmetric-research` and write `workspace/briefs/crypto-asymmetric.md`.

## Inputs
- `workspace/briefs/scan.md` (required for Track A)
- `workspace/briefs/rh-basis.md`
- `workspace/strategies.md` (**starter default:** PortfolioStrategy; Morpho/CL only if owner opted into advanced)
...
| **B — Crypto asymmetric** | Owner opts in (advanced) | `crypto-asymmetric.md` finalists + **FORK TRANSLATION** section; proxy maps only when mechanism is real |
...
- For Morpho/CL sleeves (advanced only): yield/pool thesis + unwind risk (not equity FA cosplay)
...
If the lens defaults to `no-trade`, do **not** stop on beta. Write `workspace/briefs/basket-options.md`: **3–5 scored options** from fork-eligible universe (PortfolioStrategy first; Morpho/CL sleeve options only if advanced opted in), within strategy caps. Owner picks.
...
- Do not default to Morpho/CL or dual-track — starter is PortfolioStrategy Track A
